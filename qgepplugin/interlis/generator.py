...
template = env.get_template('generator.model.py.tpl')
result = template.render({
    'TABLE_MAPPING': TABLES_MAPPINGS['qgep'],
    'model': 'qgep',
    'ilimodel': 'abwasser',
})

open(os.path.join(os.path.dirname(__file__), 'generator.model.py'), 'w', newline='\n').write(result)


# def generate(model='qgep'):
#     ilimodel = 'abwasser' if model == 'qgep' else 'wasser'
#     MODEL = QGEP if model == 'qgep' else QWAT
#     ILIMODEL = ABWASSER if model == 'qgep' else WASSER

#     template_path = os.path.join(os.path.dirname(__file__), f'{model}.py.tpl')
#     template = open(template_path, 'w', newline='\n')
#     generator_path = os.path.join(os.path.dirname(__file__), f'generator.{model}.py.tpl')
...
#     TABLE_MAPPING = TABLES_MAPPINGS[model]

#     for model_class, interlis_classes in TABLE_MAPPING.items():

#         available_fields = collections.defaultdict(list)
#         for attr_name, attr in list(model_class.__dict__.items()):
#             # if attr_name.startswith('__'):
#             #     continue
#             if not isinstance(attr, InstrumentedAttribute):
#                 continue
...
#                 key = attr.property.columns[0].table.name
#             available_fields[key].append(attr_name)
#         ordered_tables = ["_relations_"]+list(c.__table__.name for c in model_class.__mro__ if hasattr(c, '__table__'))
#         available_fields = sorted(available_fields.items(), key=lambda i: ordered_tables.index(i[0]), reverse=True)

#         classes = ", ".join(f"WASSER.{c.__name__}" for c in interlis_classes)
#         template.write(f'    print("Exporting {model.upper()}.{model_class.__name__} -> {classes}")\n')
#         template.write(f'    for row in session.query({model.upper()}.{model_class.__name__}):\n')
#         template.write(f'        # AVAILABLE FIELDS IN {model.upper()}.{model_class.__name__}\n\n')
#         for src_table, fields in available_fields:
#             fields.sort()
#             template.write(f'        # --- {src_table} ---\n')
#             template.write(f'        # {", ".join(fields)}\n\n')

#         for interlis_class in interlis_classes:

#             fields_to_map = collections.defaultdict(list)
#             for attr_name, attr in list(interlis_class.__dict__.items()):
#                 # if attr_name.startswith('__'):
#                 #     continue
#                 if not isinstance(attr, InstrumentedAttribute):
#                     continue
...
#             ordered_tables = list(c.__table__.name for c in interlis_class.__mro__ if hasattr(c, '__table__'))
#             fields_to_map = sorted(fields_to_map.items(), key=lambda i: ordered_tables.index(i[0]), reverse=True)

#             template.write(f'        {interlis_class.__name__} = {ilimodel.upper()}.{interlis_class.__name__}(\n')
#             template.write(f'            # FIELDS TO MAP TO {ilimodel.upper()}.{interlis_class.__name__}\n')
...
#             template.write(f'        )\n')
#             template.write(f'        session.add({interlis_class.__name__})\n')
#         template.write(f'        print(".", end="")\n')
#         template.write(f'    print("done")\n')
#         template.write(f'    session.flush()\n\n')

#     print("\n"*5)
#     available_tables = ', '.join(sorted(c.__name__ for c in ILIMODEL if c not in TABLE_MAPPING.values()))
#     generator.write('    TABLE_MAPPING = {\n')
#     for model_class, interlis_classes in TABLE_MAPPING.items():
#         interlis_classes_str = ", ".join(f"{ilimodel.upper()}.{c.__name__}" for c in interlis_classes)
#         generator.write(f"        {model.upper()}.{model_class.__name__}: [{interlis_classes_str}],\n")
#     generator.write(f'        # NOT MAPPED YET\n')
#     generator.write(f'        # AVAILABLE {ilimodel.upper()} CLASSES : {available_tables}\n')
#     for model_class in sorted(list(MODEL), key=lambda q: q.__name__):
#         if model_class not in TABLE_MAPPING.keys():
#             generator.write(f"        # {model.upper()}.{model_class.__name__}: [{ilimodel.upper()}.REPLACE_ME],\n")
#     generator.write('    }\n\n')
