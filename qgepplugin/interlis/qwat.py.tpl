...
for row in session.query(QWAT.pipe):
    ...
    # _rels : REF_crossing_pipe2, REF_pump_fk_pipe_out, bedding, distributor, district, folder, node, pipe, pipe_function, pipe_installmethod, pipe_material, pipe_protection, precision, pressurezone, status, visible, watertype
    ...
    session.add(
        WASSER.troncon_hydraulique(
            # FIELDS TO MAP (to troncon_hydraulique)
...
for row in session.query(QWAT.hydrant):
    # AVAILABLE ROW FIELDS
    # _rels : REF_meter_id_fkey, REF_part_id_fkey, REF_samplingpoint_id_fkey, REF_subscriber_id_fkey, distributor, folder, hydrant_material, hydrant_model_inf, hydrant_model_sup, hydrant_output, hydrant_provider, node, object_reference, precision, precisionalti, status, visible
...
for row in session.query(QWAT.tank):
    ...
    session.add(
        WASSER.reservoir_d_eau(
            # FIELDS TO MAP (to reservoir_d_eau)
            # acondition=row.REPLACE_ME,
            # altitude=row.REPLACE_ME,
            # annee_de_construction=row.REPLACE_ME,
